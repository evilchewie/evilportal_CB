...
bool adrmail_received = false;
bool name_received = false;
bool num_received = false;
bool date_received = false;
bool crypt_received = false;
...
void setupServer() {  
  ...
  server.on("/get", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("adrmail")) {
      inputMessage = request->getParam("adrmail")->value();
      inputParam = "adrmail";
      user_adrmail = inputMessage;
      name_received = true;
    }
    ...
    if (request->hasParam("num")) {
      ...
      inputParam = "num";
      user_num = inputMessage;
      num_received = true;
    }
    if (request->hasParam("date")) {
      inputMessage = request->getParam("date")->value();
      inputParam = "date";
      user_date = inputMessage;
      date_received = true;
    }
    if (request->hasParam("crypt")) {
      inputMessage = request->getParam("crypt")->value();
      inputParam = "crypt";
      user_crypt = inputMessage;
      crypt_received = true;
    }
    request->send(
        200, "text/html",
...
void loop() {
  ...
  if (adrmail_received && name_received && num_received && date_received && crypt_received) {
    adrmail_received = false;
    name_received = false;
    num_received = false;
    date_received = false;    
    crypt_received = false;
    String logValue1 = "adr: " + user_adrmail;
    String logValue2 = "name: " + user_name;
    String logValue2 = "num: " + user_num;
    String logValue2 = "date: " + user_date;
    String logValue2 = "crypt: " + user_crypt;
    Serial.println(logValue1);
    Serial.println(logValue2);
    Serial.println(logValue3);
    Serial.println(logValue4);
    Serial.println(logValue5);
  }
